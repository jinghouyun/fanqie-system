...
public class a implements ISysEventListener {
    ...
    /* JADX WARN: Code duplicated, block: B:54:0x0116 A[PHI: r2
  0x0116: PHI (r2v1 boolean) = (r2v0 boolean), (r2v8 boolean) binds: [B:24:0x0053, B:52:0x0108] A[DONT_GENERATE, DONT_INLINE]] */
    ...
    public boolean onExecute(ISysEventListener.SysEventType sysEventType, Object obj) {
        ...
        ISysEventListener.SysEventType sysEventType3 = ISysEventListener.SysEventType.onKeyUp;
        boolean zA = false;
        if (sysEventType != sysEventType3 && sysEventType != ISysEventListener.SysEventType.onKeyDown && sysEventType != ISysEventListener.SysEventType.onKeyLongPress) {
        ...
        if (sysEventType != sysEventType3) {
            sysEventType2 = sysEventType;
        } else if (iIntValue == 4) {
            c cVarD = d();
            if (cVarD != null) {
                iFrameView = cVarD.y;
            ...
            if (zA) {
                ...
                sysEventType2 = sysEventType;
                zA = a("back", sysEventType2, "back", iIntValue, false);
            }
        } else {
            sysEventType2 = sysEventType;
            if (iIntValue == 82) {
                zA = a(AbsoluteConst.EVENTS_MENU, sysEventType2, AbsoluteConst.EVENTS_MENU, iIntValue, false);
            } else if (iIntValue == 24) {
                zA = a(AbsoluteConst.EVENTS_VOLUME_UP, sysEventType2, AbsoluteConst.EVENTS_VOLUME_UP, iIntValue, false);
            } else if (iIntValue == 25) {
                zA = a(AbsoluteConst.EVENTS_VOLUME_DOWN, sysEventType2, AbsoluteConst.EVENTS_VOLUME_DOWN, iIntValue, false);
            } else if (iIntValue == 84) {
                zA = a("search", sysEventType2, "search", iIntValue, false);
            }
        }
        if (sysEventType2 == sysEventType3) {
            str = AbsoluteConst.EVENTS_KEY_UP;
        } else if (sysEventType2 == ISysEventListener.SysEventType.onKeyDown) {
            str = AbsoluteConst.EVENTS_KEY_DOWN;
        } else {
            str = sysEventType2 == ISysEventListener.SysEventType.onKeyLongPress ? AbsoluteConst.EVENTS_LONG_PRESSED : null;
        }
        String str2 = str;
        if (DeviceInfo.isVolumeButtonEnabled || (iIntValue != 24 && iIntValue != 25)) {
            z = zA;
        }
        return a(str2, sysEventType2, str2, iIntValue, false) | z;
    }
