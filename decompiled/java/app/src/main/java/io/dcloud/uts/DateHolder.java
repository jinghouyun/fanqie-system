...
public final class DateHolder {
    ...
    public final Number getDateField(double dateDouble, int magic) {
        ...
        if (z2 && i == 0) {
            return Double.valueOf(dateFields[0] - ((double) 1900));
        }
    ...
    public final double[] getDateFields(double dateDouble, boolean isLocal, boolean force) {
        ...
        while (true) {
            ...
            if (i2 == 1) {
                c = 1;
                dDaysInYear += (double) (daysInYear(iYearFromDays) - 365);
            } else {
                c = 1;
            }
            d2 = d4;
    ...
    public final double setDateFields(double[] fields, boolean isLocal) {
        ...
        if (dFloor >= -271821.0d && dFloor <= 275760.0d) {
            int i = (int) dFloor;
            int i2 = (int) d5;
            long jDaysFromYear = daysFromYear(i);
            for (int i3 = 0; i3 < i2; i3++) {
                jDaysFromYear += (long) monthDays[i3];
                if (i3 == 1) {
                    jDaysFromYear += (long) (daysInYear(i) - 365);
                }
            }
            double timezoneOffset = (((jDaysFromYear + d3) - ((double) 1)) * ((double) 86400000)) + (fields[3] * ((double) 3600000)) + (fields[4] * ((double) WXRequest.DEFAULT_TIMEOUT_MS)) + (fields[5] * ((double) 1000)) + fields[6];
            if (java.lang.Math.abs(timezoneOffset) <= Double.MAX_VALUE) {
                if (isLocal) {
                    timezoneOffset += ((double) getTimezoneOffset(timezoneOffset < -9.223372036854776E18d ? Long.MIN_VALUE : timezoneOffset >= 9.223372036854776E18d ? Long.MAX_VALUE : (long) timezoneOffset)) * 60000.0d;
                }
    ...
    private final long daysFromYear(long y) {
        return (((((long) 365) * (y - ((long) 1970))) + floorDiv(y - ((long) 1969), 4L)) - floorDiv(y - ((long) 1901), 100L)) + floorDiv(y - ((long) 1601), 400L);
    }
