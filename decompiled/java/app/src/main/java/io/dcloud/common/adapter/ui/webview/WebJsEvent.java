    ...
    public void showOpenFileChooser(ValueCallback<Uri> valueCallback, ValueCallback<Uri[]> valueCallback2, String str, String str2, WebChromeClient.FileChooserParams fileChooserParams) {
        ...
        this.mAdaWebview.obtainFrameView().obtainApp().registerSysEventListener(new ISysEventListener() { // from class: io.dcloud.common.adapter.ui.webview.WebJsEvent.10
                ...
                if (Build.VERSION.SDK_INT >= 29 || !cursorQuery.moveToFirst()) {
                    ...
                    try {
                        ...
                        if (columnIndexOrThrow > -1) {
                            ...
                            try {
                                if (PdrUtil.isEmpty(uri2.getScheme())) {
                                    uri2 = Uri.parse((string.startsWith("/") ? DeviceInfo.FILE_PROTOCOL : "file:///") + string);
                                }
                            } catch (Exception unused) {
                            }
                        } else {
            ...
            public boolean onExecute(ISysEventListener.SysEventType sysEventType, Object obj) {
                ...
                if (iIntValue != 1) {
                    if (iIntValue == 2 && (list = WebJsEvent.this.dialog.uris) != null) {
                        for (File file : list) {
                            if (file.exists()) {
                                arrayList.add(FileProvider.getUriForFile(WebJsEvent.this.mAdaWebview.getActivity(), WebJsEvent.this.mAdaWebview.getActivity().getPackageName() + ".dc.fileprovider", file));
                                break;
                            }
                        }
                    }
        ...
        if (jSONObject != null) {
            try {
                if (jSONObject.has("titleText")) {
                    ...
                    obj = jSONObject.has("titletext") ? jSONObject.get("titletext") : null;
                }
        ...
        if (frameLayoutObtainActivityContentView != null) {
            ...
            if ("landscape".equals(webviewProperty)) {
                ...
                if ("landscape-secondary".equals(webviewProperty)) {
                    ...
                    requestedOrientation = "portrait-secondary".equals(webviewProperty) ? 9 : activity.getRequestedOrientation();
                }
            }
