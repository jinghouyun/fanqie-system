...
public class AppStatusBarManager {
    ...
    public AppStatusBarManager(Activity activity, e5 e5Var) {
        initStatusBarDefaultColor(activity);
        this.mWebAppInfo = e5Var;
        this.mIApp = (IApp) e5Var;
        initDirectImmersive();
    }
    ...
    public boolean checkImmersedStatusBar(Context context, boolean z) {
        ...
        try {
            ...
            z2 = (applicationInfo == null || (bundle = applicationInfo.metaData) == null) ? false : bundle.getBoolean("immersed.status.bar", false);
        ...
        this.isImmersive = z3;
        if (!z3 && this.mWebAppInfo.getActivity().getIntent().hasExtra(IntentConst.DIRECT_PAGE) && BaseInfo.isWap2AppAppid(this.mWebAppInfo.o) && !this.mIApp.manifestBeParsed()) {
            JSONObject directStatusJson = getDirectStatusJson(this.mIApp);
            if (directStatusJson == null || !directStatusJson.has(AbsoluteConst.JSONKEY_STATUSBAR_IMMERSED)) {
                this.isImmersive = true;
            } else {
