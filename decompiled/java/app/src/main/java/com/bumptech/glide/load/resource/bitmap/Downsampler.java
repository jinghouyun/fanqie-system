...
public final class Downsampler {
        ...
        if (shouldUsePool(imageType)) {
            if (i3 < 0 || i4 < 0 || !z2) {
                ...
                if (Log.isLoggable(str, 2)) {
                    Log.v(str, "Calculated target [" + iRound2 + Constants.Name.X + iRound + "] for source [" + i3 + Constants.Name.X + i4 + "], sampleSize: " + i9 + ", targetDensity: " + options.inTargetDensity + ", density: " + options.inDensity + ", density multiplier: " + f);
                }
                i6 = iRound2;
            } else {
                str = TAG;
    ...
    private static void calculateScaling(ImageHeaderParser.ImageType imageType, ImageReader imageReader, DecodeCallbacks decodeCallbacks, BitmapPool bitmapPool, DownsampleStrategy downsampleStrategy, int i, int i2, int i3, int i4, int i5, BitmapFactory.Options options) throws IOException {
        ...
        if (Log.isLoggable(TAG, 2)) {
            Log.v(TAG, "Calculate scaling, source: [" + i2 + Constants.Name.X + i3 + "], degreesToRotate: " + i + ", target: [" + i4 + Constants.Name.X + i5 + "], power of two scaled: [" + iFloor + Constants.Name.X + iRound + "], exact scale factor: " + scaleFactor + ", power of 2 sample size: " + iMax + ", adjusted scale factor: " + scaleFactor2 + ", target density: " + options.inTargetDensity + ", density: " + options.inDensity);
        }
    }
