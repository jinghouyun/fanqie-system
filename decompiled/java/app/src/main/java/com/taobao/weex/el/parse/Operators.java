    ...
    public static boolean isEquals(Token token, Token token2, Object obj) {
        ...
        if (objExecute instanceof Number) {
            if (objExecute2 instanceof Number) {
                return ((Number) objExecute).doubleValue() == ((Number) objExecute2).doubleValue();
            }
            return ((Number) objExecute).doubleValue() == getNumber(objExecute2);
        }
        if (objExecute2 instanceof Number) {
            return getNumber(objExecute) == ((Number) objExecute2).doubleValue();
        }
        return ((objExecute instanceof CharSequence) || (objExecute2 instanceof CharSequence)) ? objExecute.toString().trim().equals(objExecute2.toString().trim()) : objExecute.equals(objExecute2);
    }
