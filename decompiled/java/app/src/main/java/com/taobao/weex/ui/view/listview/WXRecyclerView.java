...
public class WXRecyclerView extends RecyclerView implements WXGestureObservable {
        ...
        if (i == 1 && i == 1) {
            try {
                if (this.NestInfo != null && this.isNestParent) {
                    ...
                    return childRecylerView.getHeight() - rect.bottom != 0 || childRecylerView.isScrollTop();
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private WXRecyclerView getChildRecylerView() {
        ...
        DCWXSlider dCWXSliderComponent;
        JSONObject jSONObject = this.NestInfo;
        if (jSONObject == null || !this.isNestParent) {
            return null;
        }
        if (jSONObject.getBooleanValue("isSwipelist")) {
            wXComponentById = WXSDKManager.getInstance().getWXRenderManager().getWXComponentById(this.mInstanceId, this.NestInfo.getString("swipeId"));
            if (wXComponentById != null && (dCWXSliderComponent = getDCWXSliderComponent(wXComponentById)) != null) {
                wXComponent = wXComponentById;
