...
public class FileUtils {
    ...
    private static String getNameOfFileInZip(File file, String str) throws IOException {
        ...
        return isSymbolicLink(file) ? file.toPath().toRealPath(LinkOption.NOFOLLOW_LINKS).getFileName().toString() : file.getName();
    }
    ...
    private static void applyWindowsFileAttributes(Path path, byte[] bArr) {
        DosFileAttributeView dosFileAttributeViewM1866m;
        if (bArr[0] == 0 || (dosFileAttributeViewM1866m = StreamsKt$$ExternalSyntheticApiModelOutline0.m1866m((Object) Files.getFileAttributeView(path, FileUtils$$ExternalSyntheticApiModelOutline2.m(), LinkOption.NOFOLLOW_LINKS))) == null) {
            return;
        }
        try {
            dosFileAttributeViewM1866m.setReadOnly(BitUtils.isBitSet(bArr[0], 0));
            dosFileAttributeViewM1866m.setHidden(BitUtils.isBitSet(bArr[0], 1));
            dosFileAttributeViewM1866m.setSystem(BitUtils.isBitSet(bArr[0], 2));
            dosFileAttributeViewM1866m.setArchive(BitUtils.isBitSet(bArr[0], 5));
    ...
    private static byte[] getWindowsFileAttributes(Path path) {
        byte[] bArr = new byte[4];
        try {
            DosFileAttributeView dosFileAttributeViewM1866m = StreamsKt$$ExternalSyntheticApiModelOutline0.m1866m((Object) Files.getFileAttributeView(path, FileUtils$$ExternalSyntheticApiModelOutline2.m(), LinkOption.NOFOLLOW_LINKS));
            if (dosFileAttributeViewM1866m != null) {
                DosFileAttributes attributes = dosFileAttributeViewM1866m.readAttributes();
