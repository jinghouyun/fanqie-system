...
public final class BroadcastChannelImpl<E> extends BufferedChannel<E> implements BroadcastChannel<E> {
        ...
        try {
            ...
            if (!isClosedForSend() || this.lastConflatedElement != BroadcastChannelKt.NO_ELEMENT) {
                if (this.lastConflatedElement != BroadcastChannelKt.NO_ELEMENT) {
                    subscriberConflated.mo2084trySendJP2dKIU(getValue());
                }
    ...
    public Object mo2084trySendJP2dKIU(E element) {
        ...
        try {
            ...
            if (!(list instanceof Collection) || !list.isEmpty()) {
                ...
                while (it.hasNext()) {
                    if (((BufferedChannel) it.next()).shouldSendSuspend$kotlinx_coroutines_core()) {
                        Object objM2109failurePtdJZtk = ChannelResult.INSTANCE.m2109failurePtdJZtk();
                        reentrantLock.unlock();
                        return objM2109failurePtdJZtk;
                    }
                }
            }
            if (this.capacity == -1) {
            ...
            while (it2.hasNext()) {
                ((BufferedChannel) it2.next()).mo2084trySendJP2dKIU(element);
            }
            Object objM2110successJP2dKIU = ChannelResult.INSTANCE.m2110successJP2dKIU(Unit.INSTANCE);
            reentrantLock.unlock();
            return objM2110successJP2dKIU;
        } catch (Throwable th) {
            reentrantLock.unlock();
            throw th;
        }
