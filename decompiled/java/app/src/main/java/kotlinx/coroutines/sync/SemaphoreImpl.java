...
public class SemaphoreImpl implements Semaphore {
    ...
    private final <W> void acquire(W waiter, Function1<? super W, Boolean> suspend, Function1<? super W, Unit> onAcquired) {
        while (decPermits() <= 0) {
            if (suspend.invoke2(waiter).booleanValue()) {
                return;
            }
        }
        onAcquired.invoke2(waiter);
    }
    ...
    public final boolean addAcquireToQueue(Waiter waiter) {
        ...
        SemaphoreSegment semaphoreSegment2 = (SemaphoreSegment) SegmentOrClosed.m2146getSegmentimpl(objFindSegmentInternal);
