...
public final class Response implements Closeable {
    ...
    public ResponseBody peekBody(long j) throws IOException {
        ...
        Buffer bufferM421clone = bufferedSourceSource.buffer().m421clone();
        if (bufferM421clone.size() > j) {
            Buffer buffer = new Buffer();
            buffer.write(bufferM421clone, j);
            bufferM421clone.clear();
            bufferM421clone = buffer;
        }
        return ResponseBody.create(this.body.contentType(), bufferM421clone.size(), bufferM421clone);
    }
