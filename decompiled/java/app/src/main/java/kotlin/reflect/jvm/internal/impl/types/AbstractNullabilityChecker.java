        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(rigidTypeMarkerPop)) {
                ...
                if (none == null) {
                    ...
                    while (it.hasNext()) {
                        RigidTypeMarker rigidTypeMarkerMo1824transformType = none.mo1824transformType(typeCheckerState, it.next());
                        if ((typeSystemContext.isClassType(rigidTypeMarkerMo1824transformType) && !typeSystemContext.isMarkedNullable(rigidTypeMarkerMo1824transformType)) || typeSystemContext.isDefinitelyNotNullType(rigidTypeMarkerMo1824transformType)) {
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(rigidTypeMarkerPop)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(rigidTypeMarkerPop)).iterator();
                    while (it.hasNext()) {
                        RigidTypeMarker rigidTypeMarkerMo1824transformType = supertypesPolicy.mo1824transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, rigidTypeMarkerMo1824transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(rigidTypeMarkerMo1824transformType);
