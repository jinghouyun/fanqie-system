    ...
    protected FunctionDescriptor doSubstitute(CopyConfiguration copyConfiguration) {
        ...
        if (!copyConfiguration.newContextReceiverParameters.isEmpty()) {
            ...
            for (ReceiverParameterDescriptor receiverParameterDescriptor2 : copyConfiguration.newContextReceiverParameters) {
                KotlinType kotlinTypeSubstitute = typeSubstitutorSubstituteTypeParameters.substitute(receiverParameterDescriptor2.getType(), Variance.IN_VARIANCE);
                if (kotlinTypeSubstitute == null) {
                    return null;
                }
                int i2 = i + 1;
                arrayList2.add(DescriptorFactory.createContextReceiverParameterForCallable(functionDescriptorImplCreateSubstitutedCopy, kotlinTypeSubstitute, ((ImplicitContextReceiver) receiverParameterDescriptor2.getValue()).getCustomLabelName(), receiverParameterDescriptor2.getAnnotations(), i));
                zArr[0] = zArr[0] | (kotlinTypeSubstitute != receiverParameterDescriptor2.getType());
                i = i2;
            }
        }
        ...
        KotlinType kotlinTypeSubstitute3 = typeSubstitutorSubstituteTypeParameters.substitute(copyConfiguration.newReturnType, Variance.OUT_VARIANCE);
        if (kotlinTypeSubstitute3 == null) {
            return receiverParameterDescriptor;
        }
        boolean z = zArr[0] | (kotlinTypeSubstitute3 != copyConfiguration.newReturnType);
        zArr[0] = z;
        if (!z && copyConfiguration.justForTypeSubstitution) {
            return this;
        }
        functionDescriptorImplCreateSubstitutedCopy.initialize(receiverParameterDescriptorImpl, receiverParameterDescriptor, arrayList2, arrayList, substitutedValueParameters, kotlinTypeSubstitute3, copyConfiguration.newModality, copyConfiguration.newVisibility);
