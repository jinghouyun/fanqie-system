...
public class TypeSubstitutor {
    ...
    private TypeProjection unsafeSubstitute(TypeProjection typeProjection, TypeParameterDescriptor typeParameterDescriptor, int i) throws SubstitutionException {
        ...
        if (!typeProjection.isStarProjection()) {
            ...
            if (!DynamicTypesKt.isDynamic(type) && !(type.unwrap() instanceof RawType)) {
                TypeProjection typeProjectionMo1825get = this.substitution.mo1825get(type);
                TypeProjection typeProjectionProjectedTypeForConflictedTypeWithUnsafeVariance = typeProjectionMo1825get != null ? projectedTypeForConflictedTypeWithUnsafeVariance(type, typeProjectionMo1825get, typeParameterDescriptor, typeProjection) : null;
    ...
    private TypeProjection substituteCompoundType(TypeProjection typeProjection, int i) throws SubstitutionException {
        KotlinType type = typeProjection.getType();
        Variance projectionKind = typeProjection.getProjectionKind();
        if (type.getConstructor().mo1820getDeclarationDescriptor() instanceof TypeParameterDescriptor) {
            return typeProjection;
        }
