...
public final class AbstractTypeChecker {
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(rigidTypeMarkerPop)) {
                ...
                if (lowerIfFlexible == null) {
                    ...
                    while (it.hasNext()) {
                        RigidTypeMarker rigidTypeMarkerMo1824transformType = lowerIfFlexible.mo1824transformType(typeCheckerState, it.next());
                        if (typeSystemContext.isNothingConstructor(typeSystemContext.typeConstructor(rigidTypeMarkerMo1824transformType))) {
                            typeCheckerState.clear();
                            return true;
                        }
                        supertypesDeque.add(rigidTypeMarkerMo1824transformType);
                    }
                }
            }
        }
    ...
    private final boolean isSubtypeOfForSingleClassifierType(final TypeCheckerState typeCheckerState, RigidTypeMarker rigidTypeMarker, final RigidTypeMarker rigidTypeMarker2) {
        ...
        if (z || !INSTANCE.isSubtypeForSameConstructor(typeCheckerState, argumentList, rigidTypeMarker2)) {
            return typeCheckerState.runForkingPoint(new Function1(linkedHashSet, typeCheckerState, typeSystemContext, rigidTypeMarker2) { // from class: kotlin.reflect.jvm.internal.impl.types.AbstractTypeChecker$$Lambda$0
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(rigidTypeMarkerPop)) {
                ...
                if (lowerIfFlexibleSubstitutionSupertypePolicy != null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(rigidTypeMarkerPop)).iterator();
                    while (it.hasNext()) {
                        supertypesDeque.add(lowerIfFlexibleSubstitutionSupertypePolicy.mo1824transformType(typeCheckerState, it.next()));
                    }
                }
            }
        }
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(rigidTypeMarkerPop)) {
                ...
                if (lowerIfFlexible != null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(rigidTypeMarkerPop)).iterator();
                    while (it.hasNext()) {
                        supertypesDeque.add(lowerIfFlexible.mo1824transformType(state, it.next()));
                    }
                }
            }
        }
