    ...
    private final int decodeImpl(byte[] source, byte[] destination, int destinationOffset, int startIndex, int endIndex) {
        ...
        while (true) {
            ...
            if (i5 != i3 || iHandlePaddingSymbol + 3 >= endIndex) {
                ...
                int i8 = (iArr[source[iHandlePaddingSymbol + 1] & 255] << 12) | (iArr[source[iHandlePaddingSymbol] & 255] << 18) | (iArr[source[iHandlePaddingSymbol + 2] & 255] << 6) | iArr[source[iHandlePaddingSymbol + 3] & 255];
                if (i8 >= 0) {
        ...
        if (i5 != -8 && !z && this.paddingOption == PaddingOption.PRESENT) {
        ...
        int i13 = source[iSkipIllegalSymbolsIfMime] & 255;
